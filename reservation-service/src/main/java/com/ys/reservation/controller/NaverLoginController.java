...
public class NaverLoginController {
	...
	public String login(HttpSession session) {
		if(session.getAttribute("login") != null) {
			...
			try {
				callback = URLEncoder.encode("http://localhost:8080/oauth2callback", "UTF-8");
			} catch (UnsupportedEncodingException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			return "redirect:https://nid.naver.com/oauth2.0/authorize?client_id=" + clientId + "&response_type=code&redirect_uri="
	...
	public String oauth2callback(HttpSession session, @RequestParam String state, @RequestParam String code) {
		...
		if( !state.equals( storedState ) ) {
			...
			ResponseEntity<NaverProfileResponse> profileResponseEntity = restTemplate.exchange(url, HttpMethod.GET, entity, NaverProfileResponse.class);
