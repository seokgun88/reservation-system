...
public class ImageDao {
	...
	private RowMapper<Image> imageRowMapper = BeanPropertyRowMapper
			.newInstance(Image.class);
	private RowMapper<ReviewImageVo> reviewImageVoRowMapper = BeanPropertyRowMapper
			.newInstance(ReviewImageVo.class);

	public ImageDao(DataSource dataSource) {
		this.jdbc = new NullableNamedParameterJdbcTemplate(dataSource);
		this.insertAction = new SimpleJdbcInsert(dataSource)
				.withTableName("files").usingGeneratedKeyColumns("id");
	}
	...
	public List<Integer> selectIdsByProductId(int productId) {
		Map<String, Integer> param = Collections.singletonMap("productId",
				productId);
		return jdbc.queryForList(ImageSqls.SELECT_BY_PRODUCT_ID, param,
				Integer.class);
	}

	public List<ReviewImageVo> selectIdsByReviewIds(List<Integer> reviewIds) {
		Map<String, List<Integer>> param = Collections.singletonMap("reviewIds",
				reviewIds);
		return jdbc.query(ImageSqls.SELECT_BY_REVIEW_IDS, param,
				reviewImageVoRowMapper);
	}
